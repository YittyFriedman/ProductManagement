...
$(document).ready(function () {
    ...
    $(document).on('click', ".ckSupplier", function () {
        ...
        var supplierDesc = $(this).val();
        $(".ckSupplier").prop("checked", false);
    ...
    $(document).on('click', "#btnAddSupplier", function () {
        $("#spnModalTitle").text("Supplier");
        $("#lblModelName").text("Supplier Name:");
        $("#divModelCode").hide();
    ...
    $(document).on('click', ".ckType", function () {
        ...
        var typeDesc = $(this).val();
        $(".ckType").prop("checked", false);
    ...
    $(document).on('click', ".ckSizeTitle", function () {
        ...
        var titleDesc = $(this).val();
        $(".ckSizeTitle").prop("checked", false);
    ...
    $(document).on('click', ".ckOrientation", function () {
        ...
        if (checked) {
            ...
            else { $("#lblSize").text(window.orientationOptionsCount + " selected"); }
    ...
    $(document).on('click', "#btnSaveNewUOM", function () {
        ...
        alert("Please enter a valid 3 character UOM Code");
    }
    });
        ...
        $.ajax({
            ...
            success: function (res) {
                if (res.supplierList.length > 0) {
                    ...
                    $('#ddmSupplier').append(Shared.AddDropDownButton("btnAddSupplier", "Add Supplier"));
                    $("#lblSupplier").text("Select one");
                    window.supplierId = 0;
                    window.supplierDescription = "";
                }
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                if (res.classList.length > 0) {
                    for (var i = 0; i < res.classList.length; i++) {//level 1
                      var level1 = res.classList[i];
                        if (level1.ClassChildren.length > 0) {
                            $('#ddmClass').append('<li class="dropdown-submenu"><a href="#" class="test" tabindex = "-1">' + level1.Description + '<span class="caret" ></span ></a><ul class="dropdown-menu" id="ddmClassLevel2'+i+'">');

                            for (var j = 0; j < level1.ClassChildren.length; j++) {
                                var level2 = level1.ClassChildren[j];
                                if (level2.ClassChildren.length > 0) {
                                    $('#ddmClassLevel2' + i).append('<li class="dropdown-submenu"><a href="#" class="test" tabindex = "-1">' + level2.Description + '<span class="caret" ></span ></a><ul class="dropdown-menu" id="ddmClassLevel3'+i+j+'">');

                                    for (var k = 0; k < level2.ClassChildren.length; k++) {
                                        var level3 = level2.ClassChildren[k];
                                        if (level3.ClassChildren.length >0) {
                                            $('#ddmClassLevel3' + i + j).append('<li class="dropdown-submenu"><a href="#" class="test" tabindex = "-1">' + level3.Description + '<span class="caret" ></span ></a><ul class="dropdown-menu" id="ddmClassLevel4'+i+j+k+'">');

                                            for (var l = 0; l < level3.ClassChildren.length; l++) {
                                                var level4 = level3.ClassChildren[l];
                                                $('#ddmClassLevel4' + i + j + k).append('<li><a href="#" class="liClass" id="liClass' + level4.RevNo + '" data-revno="' + level4.RevNo + '" tabindex="-1">' + level4.Description + '</a></li>');
                                            }
                                            $('#ddmClassLevel3'+i+j).append('</ul>');

                                        }
                                        else {
                                            $('#ddmClassLevel3' + i + j).append('<li><a href="#" class="liClass" id="liClass' + level3.RevNo + '" data-revno="' + level3.RevNo + '" tabindex="-1">' + level3.Description + '</a></li>');
                                        }
                                    }
                                    $('#ddmClassLevel3'+i+j).append('</ul>');
                                }
                                else {
                                    $('#ddmClassLevel2' + i).append('<li><a href="#" class="liClass" id="liClass' + level2.RevNo + '" data-revno="' + level2.RevNo + '" tabindex="-1">' + level2.Description + '</a></li>');
                                }
                            }

                            $('#ddmClassLevel2'+i).append('</ul>');
                        }
                        else {
                            $('#ddmClass').append('<li><a href="#" class="liClass"  id="liClass' + level1.RevNo + '" data-revno="' + level1.RevNo + '" tabindex="-1">' + level1.Description + '</a></li>');
                        }
                    ...
                    //    res.classList[i];
                    //    var checkBoxId = "ckClass" + res.classList[i].ClassId;
                    //    var id = res.classList[i].ClassId;
                    //    var checkBoxValue = res.classList[i].ClassDescription;
                    //    var spanValue = res.classList[i].ClassDescription;
                    //    var checkboxClass = "ckClass";
                    //    $('#ddmClass').append(Shared.AddDropDownOption(checkboxClass, checkBoxId, checkBoxValue, spanValue, id));
                    //}

                    //$('#ddmClass').append(Shared.AddDropDownButton("btnAddClass", "Add Class"));
                    //$("#lblClass").text("Select one");
                    //window.classId = 0;
                    //window.classDescription = "";
                }
                ...
                if (res.productList.length > 0) {
                    ...
                    for (var i = 0; i < res.productList.length; i++) {
                        ...
                        if (isManual) {
                            ...
                            Shared.AddTableRow(i, res.productList[i].ProductSku, res.productList[i].ProductDescription);
                        }
                    }
                    $('.ddmUOMCategory').each(function () {
                        ...
                        for (var j = 0; j < newUOMCatgList.length; j++) {
                            ...
                            $(this).append(Shared.AddUOMddOption(newUOMCatgList[j].UOMCode, newUOMCatgList[j].UOMCategoryId, this.dataset.productid))
                        }
                    });
        ...
        $.ajax({
            ...
            success: function (res) {
                if (res.result != "success") {
                    alert("Error while saving new Supplier: \n" + res.result);
                }
                else {
...
var PRD = {
    CollectFormData() {
        ...
        productData.ClassId = window.classId;
        productData.ClassDescription = window.classDescription;
    ...
    ValidateProductData: function () {
        ...
        if (window.classId == 0) { $("#divClass").addClass('has-error'); invalidCount++; }
        else { $("#divClass").removeClass('has-error'); }
    ...
    CollectTableData: function () {
        ...
        for (var i = 0; i < window.productList.length; i++) {
            ...
            window.productList[i].ClassId = window.classId;
...
var Shared = {
    AddDropDownOption: function (checkboxClass, checkboxId, checkboxValue, spanValue, id, description) {
        var option = '<li><a><input type = "checkbox" class= "form-check ' + checkboxClass + '" style = "display:inline" id = "' + checkboxId + '" value = "' + checkboxValue + '" data-id = "' + id + '"  />'
            + '<span style = "display:inline; padding-left:2px" class="dropdown-item-text" > ' + spanValue + '</span></a></li > ';
        return option;
    },
    ...
    AddTableRow: function (i, productIdValue, productDesc) {
        $('#tblProducts > tbody:last-child').append('<tr>'
            ...
            + '<td> <div style="float:left;width:75%;"><input type="text" class="form-control txtUPC" id="txtUPC' + i + '" data-id="'+i+'"  /></div>'
            +'<div style = "float:right;width:20%;padding-top:10px " ><input type="checkbox" class="form-check ckNoUPC" title="No UPC found"  id="ckNoUPC'+i+'" /></div ></td>'
            + '<td><input type="text"class="form-control txtVendorId" id="txtVendorId' + i + '"  value="" /></td>'
            + '<td><input type="number" class="form-control txtCost" min="0" id="txtCost' + i + '" value="" /></td>'
            + '<td><div class="input-group"><input type="number" min="0" class="form-control txtWeight" id="txtWeight' + i + '" value="" />' + Shared.AddUOMDD("Weight" + i,"W") + '</div ></td>'
    ...
    AddUOMddOption: function (value, id, productId, isModal = false) {
        var li = '<li class="liUOM" id="liUOM' + id + '" value="' + id + '" data-productid="' + productId + '"data-ismodal="'+isModal+'">' + value + '</li>'
        return li;
    },
